...
# Helper function for parameter categorization
categorize_parameter <- function(param_name) {
  if (is.null(param_name) || is.na(param_name) || !is.character(param_name)) {
    return("Other")
  }
  ...
  if (grepl("image|xray|picture|scan|radiograph|retina|eye|optic|lens|cornea|pupil|chamber depth", param_lower))
    return("Image")
  ...
  if (grepl("blood|immune|cell|hemoglobin|platelet|lymph", param_lower))
    return("Blood/Immune System")
  ...
  if (grepl("metabolism|biochem|glucose|lipid", param_lower))
    return("Metabolism & Biochemistry")
  
  if (grepl("bone|skeletal|muscle|joint", param_lower))
    return("Musculoskeletal")
  
  if (grepl("development|morphology|embryo|structure", param_lower))
    return("Development & Morphology")
  
  if (grepl("skin|hair|nail|dermis", param_lower))
    return("Integumentary System")
  
  if (grepl("physiolog|measure|vital|function", param_lower))
    return("Physiological Measures")
...
merged_data[, category := sapply(parameter_name, categorize_parameter)]
...
server <- function(input, output, session) {
  ...
  output$heatmap <- renderPlot({
    ...
    tryCatch({
      # Create color palette from white to red
      ...
      # Create the heatmap with labeled axes and legend
      pheatmap(clust_data$correlation_matrix,
               ...
               main = title,
               legend = TRUE,
               legend_labels = c("No correlation", "Strong correlation"),
               legend_breaks = c(0, 1))
    }, error = function(e) {
      plot.new()
